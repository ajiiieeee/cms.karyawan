...
@section('content')
    <div class="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
      <div>
        <h6 class="fw-semibold mb-0">Menu Management</h6>
        <p class="text-neutral-600 mt-4 mb-0">Settings &raquo; Menu Management</p>
      </div>
      @if(Auth::user()->hasMenuAccess('menu-management', 'add'))
      <a href="{{ route('menu-management.create') }}" class="btn btn-primary-600 d-inline-flex align-items-center gap-2">
...
$(document).ready(function() {
    var table = $('#menu-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("menu-management.data") }}',
            type: 'GET',
    ...
    $(document).on('click', '.btn-delete', function() {
        ...
        Swal.fire({
            html: '<div class="delete-icon-wrapper"><i class="ri-delete-bin-6-line"></i></div>'
                + '<div class="delete-title">Hapus Menu</div>'
                + '<div class="delete-text">Anda yakin ingin menghapus menu <strong>' + name + '</strong>? Data yang dihapus tidak dapat dikembalikan.</div>'
            ...
            didOpen: function(popup) {
                ...
                popup.querySelector('#swal-confirm').addEventListener('click', function() {
                    ...
                    form.attr('action', '{{ route("menu-management.index") }}/' + id);
                    form.submit();
                });
            }
        });
