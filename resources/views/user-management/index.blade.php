...
@section('content')
    <div class="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
      <div>
        ...
        <p class="text-neutral-600 mt-4 mb-0">Settings &raquo; User Management</p>
      </div>
      @if(Auth::user()->hasMenuAccess('user-management', 'add'))
      <a href="{{ route('user-management.create') }}" class="btn btn-primary-600 d-inline-flex align-items-center gap-2">
    ...
    <div id="table-container" class="card shadow-1 radius-8 d-none">
      <div class="card-body p-24">
        <div class="table-responsive">
          <table id="user-table" class="table table-striped table-hover" style="width: 100%">
            <thead>
              <tr>
                <th width="50">No</th>
                <th>Nama</th>
    ...
    $(document).on('click', '.btn-delete', function() {
        ...
        Swal.fire({
            html: '<div class="delete-icon-wrapper"><i class="ri-delete-bin-6-line"></i></div>'
                + '<div class="delete-title">Hapus User</div>'
                + '<div class="delete-text">Anda yakin ingin menghapus user <strong>' + name + '</strong>? Data yang dihapus tidak dapat dikembalikan.</div>'
            ...
            didOpen: function(popup) {
                ...
                popup.querySelector('#swal-confirm').addEventListener('click', function() {
                    ...
                    form.attr('action', '{{ route("user-management.index") }}/' + id);
                    form.submit();
                });
            }
        });
