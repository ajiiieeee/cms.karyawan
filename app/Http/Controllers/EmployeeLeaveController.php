...
class EmployeeLeaveController extends EmployeeController
{
    ...
    {
        return view('leave-requests.index', ['requests' => PengajuanCuti::with('kategoriCuti')->where('karyawan_id', $this->employee()->id)->latest()->get()]);
    }
    public function create(): View
    {
        return view('leave-requests.form', ['requestItem' => new PengajuanCuti(), 'categories' => KategoriCuti::orderBy('nama_kategori')->get()]);
    }
    ...
    public function edit(PengajuanCuti $leave_request): View
    {
        ...
        return view('leave-requests.form', ['requestItem' => $leave_request, 'categories' => KategoriCuti::orderBy('nama_kategori')->get()]);
    }
    public function update(Request $request, PengajuanCuti $leave_request): RedirectResponse
    {
        ...
        abort_unless($leave_request->status_pengajuan === 'Pending', 403);
        $data = $request->validate(['tanggal_awal' => 'required|date', 'tanggal_akhir' => 'required|date|after_or_equal:tanggal_awal', 'jenis_cuti' => 'required|exists:kategori_cuti,id', 'keterangan' => 'nullable|string|max:1000']);
        $data['jumlah_hari'] = (string)(Carbon::parse($data['tanggal_awal'])->diffInDays(Carbon::parse($data['tanggal_akhir'])) + 1);
        $leave_request->update($data + ['updated_date' => now()->toDateString(), 'updated_by' => auth()->user()->nama]);
